 ...
 *
 *  MACH_MSGH_BITS_CIRCULAR should be zero; it is used internally.
 *
...
#define	MACH_MSGH_BITS_UNUSED		0x3fff0000
...
/*
 *  Bits for the msgh_kind header field in messages.  Note that users
 *  can send messages with any value for msgh_kind, including
 *  MACH_MSGH_KIND_NOTIFICATION if they wish.
 */
 ...
 *
 *  The msgh_kind field is uninterpreted by the message primitives.
 *  It normally carries information or attributes affecting
 *  the interpretation or delivery of the entire message.
 *  (For example, security, priority, reliability, etc.)
 *
