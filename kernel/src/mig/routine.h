 ...
 * HISTORY
 * $Log:	routine.h,v $
 * Revision 2.3  91/02/05  17:55:28  mrt
 * 	Changed to new Mach copyright
 * 	[91/02/01  17:55:21  mrt]
 * 
 * Revision 2.2  90/06/02  15:05:23  rpd
 * 	Created for new IPC.
...
#define akeMsgKind	(6)	/* pointed at by rtMsgKind */
...
#define akbReplyInit	(0001000000)	/* reply msg-type must be init'ed */
#define akbRequestQC	(0002000000)	/* msg_type can be checked quickly */
#define akbReplyQC	(0004000000)	/* msg_type can be checked quickly */
#define akbReplyCopy	(0010000000)	/* copy reply value from request */
#define akbVarNeeded	(0020000000)	/* may need local var in server */
#define akbDestroy	(0040000000)	/* call destructor function */
#define akbVariable	(0100000000)	/* variable size inline data */
#define akbWasInOut	(0200000000)	/* In arg made from an InOut arg */
 ...
 *
 * akbWasInOut helps control code generation for TrapRoutines in the
 * user-side stubs.  It means the routine had an InOut argument that was
 * split into In and Out arguments, and this is the resulting In arg.
 *
 ...
 *	akbSendBody, akbReturnRcv, akbWasInOut, akbReplyQC
 *
...
#define akMsgKind	akAddFeature(akeMsgKind, akbUserArg)
...
typedef struct routine
{
    ...
    boolean_t rtTrap;		/* TrapRoutine or TrapSimpleRoutine */
    boolean_t rtOneWay;		/* SimpleProcedure, {,Trap}SimpleRoutine */
    ...
    argument_t *rtMsgKind;	/* always non-NULL, defaults to NORMAL */

    /* fake routines, used to implement Trap routines */
    struct routine *rtFakeTrapRoutine;	/* for trap (user/server stubs) */
    struct routine *rtFakeRpcRoutine;	/* for user rpc stub */
    struct routine *rtFakeRcvRoutine;	/* for user rcv stub */
} routine_t;
...
extern routine_t *
rtMakeTrapRoutine(/* identifier_t name, argument_t *args */);
extern routine_t *
rtMakeTrapSimpleRoutine(/* identifier_t name, argument_t *args */);
