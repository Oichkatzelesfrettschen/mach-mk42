 ...
 *		-theader <name>
 *			Name the server-side trap header file <name>
 *
 *  DESIGN:
    ...
    char *argv[];
{
	...
	if ((++argv)[0][0] == '-') 
	{
		...
		break;
	      case 't':
		if (streql(argv[0], "-theader"))
		{
		    --argc; ++argv;
		    if (argc == 0)
			fatal("missing name for -theader option");
		    TrapHeaderFileName = strmake(argv[0]);
		}
		else
		    fatal("unknown flag: '%s'", argv[0]);
		break;
    ...
    char *argv[];
{
	...
	theader = myfopen(TrapHeaderFileName, "w");
    }
	...
	if (BeVerbose)
	{
	    printf("done.\nWriting %s ... ", TrapHeaderFileName);
	    fflush(stdout);
	}
	WriteTrapHeader(theader, stats);
	fclose(theader);
    }
    if (UserFilePrefix)
    {
