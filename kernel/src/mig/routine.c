    ...
    argument_t *args;
{
    ...
    if (IsCamelot)
	error("Camelot subsystems can only contain CamelotRoutines");
    ...
    argument_t *args;
{
    ...
    if (!IsCamelot)
	error("CamelotRoutines can only be used in Camelot subsystems");
    ...
    argument_t *args;
{
    ...
    rt->rtName = name;
    rt->rtKind = rkTrapRoutine;
    rt->rtArgs = args;

    if (IsCamelot)
	error("Camelot subsystems can only contain CamelotRoutines");
    if (IsKernelUser)
	error("KernelUser subsystems can't contain TrapRoutines");

    return rt;
}

routine_t *
rtMakeTrapSimpleRoutine(name, args)
    identifier_t name;
    argument_t *args;
{
    register routine_t *rt = rtAlloc();
    ...
    rt->rtName = name;
    rt->rtKind = rkTrapSimpleRoutine;
    rt->rtArgs = args;

    if (IsCamelot)
	error("Camelot subsystems can only contain CamelotRoutines");
    if (IsKernelUser)
	error("KernelUser subsystems can't contain TrapSimpleRoutines");
    ...
    routine_kind_t rk;
{
	...
	return "Function";
      case rkTrapRoutine:
	return "TrapRoutine";
      case rkTrapSimpleRoutine:
	return "TrapSimpleRoutine";
      default:
    ...
    argument_t *arg;
{
	...
	break;
      ...
      case akeMsgKind:
	if (rt->rtMsgKind != argNULL)
	    warn("multiple MsgKind args in %s; %s won't be used",
		 rt->rtName, rt->rtMsgKind->argName);
	rt->rtMsgKind = arg;
	break;
    ...
    register argument_t *arg;
{
	...
	switch(akIdent(arg->argKind))
	{
	  ...
	  case akeReplyPort:
	    ...
	    break;
	  case akeMsgKind:
	    arg->argMsgField = "Head.msgh_kind";
	    break;
	  default:
    ...
    routine_t *rt;
{
    ...
    if ((rt->rtMsgKind != argNULL) &&
	(rt->rtMsgKind->argType != itNULL))
	itCheckIntType(rt->rtMsgKind->argName,
		       rt->rtMsgKind->argType);
}
    ...
    identifier_t name;
{
    ...
    /* add msg-option after msg-type */

    if (rt->rtMsgKind != argNULL)
	loc = &rt->rtMsgKind->argNext;
    else
	loc = &rt->rtArgs;
...
/*
 *  Adds a dummy MsgKind argument to the function.
 *  This argument doesn't show up in any C argument lists;
 *  it implements the global MsgKind statement.
 */

static void
rtAddMsgKind(rt, name)
    routine_t *rt;
    identifier_t name;
{
    register argument_t *arg = argAlloc();
    ...
    arg->argName = "dummy MsgKind arg";
    arg->argVarName = name;
    arg->argType = itMsgKindType;
    arg->argKind = akeMsgKind;
    rt->rtMsgKind = arg;

    /* add msg-type after the reply port */

    if (rt->rtReplyPort != argNULL)
	loc = &rt->rtReplyPort->argNext;
    else
	loc = &rt->rtArgs;
...
static void
rtTrapCheckSpecialArgs(rt)
    register routine_t *rt;
{
    if (rt->rtWaitTime != argNULL)
	error("%s: can't have WaitTime argument", rt->rtName);
    else if (WaitTime != strNULL)
	error("%s: can't have WaitTime specification", rt->rtName);

    if (rt->rtMsgOption != argNULL)
	error("%s: can't have MsgOption argument", rt->rtName);
    else if (MsgOption != strNULL)
	error("%s: can't have MsgOption specification", rt->rtName);

    if (rt->rtMsgKind != argNULL)
	error("%s: can't have MsgKind argument", rt->rtName);
    else if (MsgKind != strNULL)
	error("%s: can't have MsgKind specification", rt->rtName);

    if ((rt->rtReplyPort != argNULL) &&
	(rt->rtKind == rkTrapSimpleRoutine))
	error("%s: can't have ReplyPort argument", rt->rtName);
}

static argument_t *
rtCopyArgList(rt, arg, mask, split)
    register routine_t *rt;
    register argument_t *arg;
    unsigned int mask;
    boolean_t split;
{
    register argument_t *next;
    register argument_t *copy;

    if (arg == argNULL)
	return arg;

    next = rtCopyArgList(rt, arg->argNext, mask, split);

    if (akCheck(arg->argKind, mask))
    {
	if (split && akCheckAll(arg->argKind, akbSend|akbReturn))
	{
	    register argument_t *copyout;

	    copy = argAlloc();
	    *copy = *arg;
	    copy->argKind = akAddFeature(akRemFeature(arg->argKind,
						      akbReturnBits|akbReply),
					 akbWasInOut);
	    copy->argVarName = strconcat(copy->argVarName, "In");

	    copyout = argAlloc();
	    *copyout = *arg;
	    copyout->argKind = akRemFeature(arg->argKind,
					    akbSendBits|akbRequest);

	    copy->argNext = copyout;
	    copyout->argNext = next;
	}
	else
	{
	    copy = argAlloc();
    ...
    if (rt->rtRequestPort == arg)
	rt->rtRequestPort = copy;
    if (rt->rtReplyPort == arg)
	rt->rtReplyPort = copy;
    if (rt->rtReturn == arg)
	rt->rtReturn = copy;
    if (rt->rtServerReturn == arg)
	rt->rtServerReturn = copy;
    if (rt->rtRetCode == arg)
	rt->rtRetCode = copy;
    if (rt->rtWaitTime == arg)
	rt->rtWaitTime = copy;
    if (rt->rtMsgOption == arg)
	rt->rtMsgOption = copy;
    if (rt->rtMsgKind == arg)
	rt->rtMsgKind = copy;
...
static routine_t *
rtCopyRoutine(rt, mask, split)
    register routine_t *rt;
    unsigned int mask;
    boolean_t split;
{
    register routine_t *new = (routine_t *) malloc(sizeof *new);

    if (new == rtNULL)
	fatal("rtCopyRoutine(): %s", unix_error_string(errno));

    *new = *rt;
    new->rtArgs = rtCopyArgList(new, rt->rtArgs, mask, split);
    return new;
}

static void
rtTrapBuildArgLists(rt)
    register routine_t *rt;
{
    char *suffix = rt->rtOneWay ? "_send" : "_rpc";
    register routine_t *fake;
    register argument_t *arg;

    /* the trap takes an explicit ReplyPort argument (if not OneWay)
       and has InOut arguments split into separate In and Out arguments. */

    fake = rtCopyRoutine(rt, akbAll, TRUE);
    rt->rtFakeTrapRoutine = fake;
    fake->rtUserName = strconcat(rt->rtUserName, "_trap");
    fake->rtServerName = strconcat(rt->rtServerName, "_trap");

    arg = fake->rtReplyPort;
    if (rt->rtOneWay)
	arg->argKind = akeReplyPort;
    else
	arg->argKind = akAddFeature(akReplyPort, akbServerArg);
    arg->argVarName = "ReplyPort";


    /* the rpc stub takes an explicit ReplyPort argument (if not OneWay)
       and a MsgOption argument */

    fake = rtCopyRoutine(rt, akbAll, FALSE);
    rt->rtFakeRpcRoutine = fake;
    fake->rtUserName = strconcat(rt->rtUserName, suffix);
    fake->rtServerName = strconcat(rt->rtServerName, suffix);

    arg = fake->rtReplyPort;
    if (rt->rtOneWay)
    {
	arg->argKind = akeReplyPort;
	arg->argVarName = "MACH_PORT_NULL";
    }
    else
    {
	arg->argKind = akReplyPort;
	arg->argVarName = "ReplyPort";
    }
    arg = fake->rtMsgOption;
    if (IsKernelServer)
    {
	arg->argKind = akMsgOption;
	arg->argVarName = "MsgOption";
    }
    else
    {
	arg->argKind = akeMsgOption;
	arg->argVarName = "MACH_MSG_OPTION_NONE";
    }


    /* the rcv stub needs real MsgOption and ReplyPort arguments */

    fake = rtCopyRoutine(rt, akbReturn|akbReply, FALSE);
    rt->rtFakeRcvRoutine = fake;
    fake->rtUserName = strconcat(rt->rtUserName, "_rcv");
    fake->rtServerName = strconcat(rt->rtServerName, "_rcv");

    if (IsKernelServer)
    {
	rtAddMsgOption(fake, "MsgOption");
	fake->rtMsgOption->argKind = akMsgOption;
    }
    else
	rtAddMsgOption(fake, "MACH_MSG_OPTION_NONE");

    rtAddDummyReplyPort(fake, itRealReplyPortType);
    fake->rtReplyPort->argVarName = "ReplyPort";
    fake->rtReplyPort->argKind = akUReplyPort;
}

static void
rtTrapCheckInts(args, mask)
    argument_t *args;
    unsigned int mask;
{
    register argument_t *arg;
	...
	if (akCheckAll(arg->argKind, mask))
	{
	    ...
	    if ((it->itTypeSize + it->itPadSize) != sizeof(int))
		error("%s: doesn't fit in register", arg->argName);
	}
}
    ...
    register routine_t *rt;
{
	...
	if (arg != rt->rtRequestPort)
	{
		...
		(it->itOutTrans != strNULL) ||
		(it->itDestructor != strNULL))
		error("%s: translation/destructor (of %s) not allowed",
		      rt->rtName, it->itName);
	}
}

/*
 * Most of these restrictions aren't inherent and could/should
 * be removed with further work on Mig.
 */
...
rtTrapCheckArgs(rt)
    register routine_t *rt;
{
    identifier_t service_type = rt->rtRequestPort->argType->itName;
    int trapuser, trapserver;

    if (!rt->rtSimpleFixedRequest || !rt->rtSimpleSendRequest)
	error("%s: request must be Simple", rt->rtName);

    if (!rt->rtSimpleCheckReply || !rt->rtSimpleReceiveReply)
	error("%s: reply must be Simple", rt->rtName);

    /* other than RequestPort, arguments can't have any translation.
       Only support certain hard-wired translations for RequestPort. */

    rtTrapCheckTrans(rt);

    if (!streql(service_type, "mach_port_t") &&
	!streql(service_type, "task_t") &&
	!streql(service_type, "thread_t") &&
	!streql(service_type, "vm_task_t") &&
	!streql(service_type, "memory_object_control_t"))
	error("%s: service port has bad type (%s)",
	      rt->rtName, service_type);

    trapuser = rtCountMask(rt->rtFakeTrapRoutine->rtArgs, akbUserArg);
    trapserver = rtCountMask(rt->rtFakeTrapRoutine->rtArgs, akbServerArg);

    if (trapuser != trapserver)
	error("%s: number of user/server trap args not equal?", rt->rtName);
    if (trapuser > 6)
	error("%s: more than six arguments to trap", rt->rtName);

    rtTrapCheckInts(rt->rtFakeTrapRoutine->rtArgs, akbSend);
}
    ...
    register routine_t *rt;
{
    ...
    rt->rtTrap = ((rt->rtKind == rkTrapRoutine) ||
		  (rt->rtKind == rkTrapSimpleRoutine));
    ...
    /* Add dummy WaitTime, MsgOption, and MsgKind arguments, if the routine
    ...
    if (rt->rtMsgKind == argNULL)
	if (MsgKind == strNULL)
	    rtAddMsgKind(rt, "MACH_MSGH_KIND_NORMAL");
	else
	    rtAddMsgKind(rt, MsgKind);
    ...
    /* Add RPC, CAMELOT bits to the msgh_kind field when appropriate. */

    if (!rt->rtOneWay)
	rt->rtMsgKind->argVarName = strconcat(rt->rtMsgKind->argVarName,
					      " | MSG_TYPE_RPC");
    if (rt->rtKind == rkCamelotRoutine)
	rt->rtMsgKind->argVarName = strconcat(rt->rtMsgKind->argVarName,
					      " | MSG_TYPE_CAMELOT");
