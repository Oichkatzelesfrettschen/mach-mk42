...
%type	<routine> TrapRoutine TrapSimpleRoutine
...
MsgKind			:	LookString syMsgKind syString
{
    if (streql($3, "MACH_MSGH_KIND_NORMAL"))
    {
	MsgKind = strNULL;
	if (BeVerbose)
...
TrapRoutine		:	syTrapRoutine syIdentifier Arguments
				{ $$ = rtMakeTrapRoutine($2, $3); }
			;

TrapSimpleRoutine	:	syTrapSimpleRoutine syIdentifier Arguments
				{ $$ = rtMakeTrapSimpleRoutine($2, $3); }
			;
