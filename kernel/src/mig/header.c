    ...
    routine_t *rt;
{
    ...
    fprintf(file, "); }\n");
    fprintf(file, "#else\n");
/*	Remove prototype generation for now 
    fprintf(file, "#if\t%s\n", NewCDecl);
    fprintf(file, "(\n");
    WriteList(file, rt->rtArgs, WriteVarDecl, akbUserArg, ",\n", "\n");
    fprintf(file, ");\n");
...
*/
    ...
    fprintf(file, "    ();\n");
/*  Remove prototype genteration for now
    fprintf(file, "#endif\n"); 
*/
    fprintf(file, "#endif\n");
}
    ...
    statement_t *stats;
{
	...
	switch (stat->stKind)
	{
	  case skRoutine:
		...
		WriteRoutine(file, stat->stRoutine->rtFakeTrapRoutine);
	    break;
	  case skImport:
	  case skUImport:
	    WriteImport(file, stat->stFileName);
	  ...
	  case skSImport:
	    break;
	  default:
	    fatal("WriteTrapHeader(): bad statement_kind_t (%d)",
		  (int) stat->stKind);
	}
}
