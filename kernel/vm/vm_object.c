...
void vm_object_init()
{
	...
	vm_object_template.pager_request = MACH_PORT_NULL;
	vm_object_template.pager_name = MACH_PORT_NULL;
	...
	register vm_object_t	object;
{
	...
	while (object != VM_OBJECT_NULL) {
		...
		"vm_object_deallocate: removing from front of cache (%x, %x)",
				vm_object_cached_list.next,
				vm_object_cached_list.prev));
	...
	register vm_object_t	object;
{
	...
	if ((object->temporary) || (object->pager == MEMORY_OBJECT_NULL)) {
		...
		if (p->dirty) {
			if (p->busy)
				panic("vm_object_terminate: busy page");
	...
	kern_return_t	reason;
{
	...
	object->pager_request = MACH_PORT_NULL;

	old_name = object->pager_name;
	object->pager_name = MACH_PORT_NULL;
	...
	vm_object_t	*_result_object;	/* OUT */
{
	...
	new_memory_object = (memory_object_t) ipc_port_alloc_kernel();
	if (new_memory_object == MACH_PORT_NULL) {
	...
	ipc_kobject_set((ipc_port_t) new_memory_object,
			(ipc_kobject_t) src_object, IKOT_PAGER);
	(void) ipc_port_make_send((ipc_port_t) new_memory_object);
	...
	ipc_port_release_send((ipc_port_t) new_memory_object);
	...
	memory_object_t	pager;
{
	...
	vm_object_cache_lock();
	switch(ip_kotype((ipc_port_t) pager)) {
		case IKOT_PAGER:
		...
		case IKOT_PAGING_REQUEST:
			object = (vm_object_t) ((ipc_port_t) pager)->ip_kobject;
			vm_object_lock(object);
	...
	mach_port_t	pager;
{
	 ...
	 */
	vm_object_cache_lock();
	if (ip_kotype((ipc_port_t) pager) != IKOT_PAGER) {
		vm_object_cache_unlock();
		return;
	}
	...
	if (old_request != MACH_PORT_NULL)
		ipc_port_dealloc_kernel((ipc_port_t) old_request);
	if (old_name != MACH_PORT_NULL)
		ipc_port_dealloc_kernel((ipc_port_t) old_name);
	...
	boolean_t	internal;
{
	...
	vm_object_cache_lock();
	while ((po = ip_kotype((ipc_port_t) pager)) == IKOT_NONE) {
		/*
		...
		if (new_object == VM_OBJECT_NULL) {
			...
			ipc_kobject_set((ipc_port_t) pager,
	...
	object = (po == IKOT_PAGER) ? (vm_object_t) ((ipc_port_t) pager)->ip_kobject : VM_OBJECT_NULL;
	...
	if (must_init) {
		...
		pager = (memory_object_t)
			ipc_port_copy_send((ipc_port_t) pager);
		if (!MACH_PORT_VALID(pager))
		...
		object->pager_request = (memory_object_t)
			ipc_port_alloc_kernel();
		if (object->pager_request == MACH_PORT_NULL)
			panic("vm_object_enter: pager request alloc");

		ipc_kobject_set((ipc_port_t) object->pager_request,
				(ipc_kobject_t) object,
				IKOT_PAGING_REQUEST);

		if (!internal /* XXX object->temporary */ ) {
			object->pager_name = (memory_object_t)
				ipc_port_alloc_kernel();
			if (object->pager_name == MACH_PORT_NULL)
				panic("vm_object_enter: pager name alloc");
		}

		/*
		 *	The Mips compiler has trouble if these assignments
		 *	are all in the same line.  (And many other compilers
		 *	generate bad code.)
		 */
		object->internal = internal;
		object->pager_ready = internal;
		object->temporary = internal;
		object->internal = internal;
	...
	vm_object_t	object;
{
	...
	if ((port = (ipc_port_t) object->pager) != IP_NULL) {
	...
	if ((port = (ipc_port_t) object->pager_request) != IP_NULL) {
		...
		if (backing_object->ref_count == 1) {
			...
			switch (vm_object_collapse_debug) {
			    case 0:
			    	break;
			    case 1:
				if ((backing_object->pager == MACH_PORT_NULL) &&
				    (backing_object->pager_request == MACH_PORT_NULL))
				    break;
				/* Fall through to... */
			...
			if (object->pager != MACH_PORT_NULL)
				ipc_kobject_set((ipc_port_t) object->pager,
						(ipc_kobject_t) object,
						IKOT_PAGER);
			...
			backing_object->pager_request = MACH_PORT_NULL;
			backing_object->pager_name = MACH_PORT_NULL;
 ...
 *		with this object.
 */
mach_port_t	vm_object_name(object)
	vm_object_t	object;
{
	ipc_port_t	p;
	...
	p = (ipc_port_t) object->pager_name;
	if (p != IP_NULL)
		p = ipc_port_make_send(p);
	vm_object_unlock(object);

	return((mach_port_t) p);
}
