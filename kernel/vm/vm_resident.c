...
#define	PMAP_MAP(size)							\
	...
	blkclr((char *) (vaddr - amount_to_map), amount_to_map);	\
	MACRO_END

	vm_page_buckets = (vm_page_bucket_t *) vaddr;
	PMAP_MAP(vm_page_bucket_count * sizeof(vm_page_bucket_t));

	for (bucket = vm_page_buckets, i = vm_page_bucket_count;
	     i--;
	     bucket++) {
...
boolean_t	vm_page_alloc_deactivate_behind = TRUE;
...
vm_page_t vm_page_grab()
{
	...
	if (vm_page_alloc_verify && !pmap_verify_free(mem->phys_addr))
		panic("vm_page_grab: page is in use!", mem);
...
void vm_page_wait()
{
	 ...
	 *	determination.  Consider: vm_fault_page might
 ...
 *	clearing the reference bit on the page.
 *
 *	The page's object and the page queues must be locked.
 */
	...
	register vm_page_t	m;
{
	...
	if (m->busy) {
		/*
		 *	We just remove the page from the page queues.
		 *	In particular, we can't clear the reference bit
		 *	because the pmap module might unmap the page,
