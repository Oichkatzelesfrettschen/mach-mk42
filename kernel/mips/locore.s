...
/*
 *	Object:
 *		tlb_umiss_cause			LOCAL variable
 *
 *	Value of c0_cause when we last entered the TLB_UMISS handler
 *	The way it is addressed requires that it be located at a
 *	positive 16 bit displacement.
 *
 *
 *	Object:
 ...
 *
 */
	ABS(tlb_umiss_cause, VEC_EXCEPTION-4)
#if	DEBUG
...
NESTED(TRAP_tlb_umiss,0,k1)		# KDB's sake
	...
	mfc0	k0,c0_cause
	lui	k1,tlb_umiss_cause>>16
	sw	k0,+tlb_umiss_cause&0x7fff(k1)
	mfc0	k0,c0_tlbcxt
	mfc0	k1,c0_epc
	lw	k0,0(k0)
#if	REF_BITS
...
VECTOR(TRAP_exception_exit,MINSAVED)
	...
	beq	AT,zero,1f
	nop				# take the AST
	lw	k0,MSS_SR(a0)		# needed in dest
	b	TRAP_generic
	li	a1,EXC_AST
...
VECTOR(TRAP_tlb_miss, (MINSAVED|MEDSAVED))
	...
	jal	save_registers
	sw	a1,MSS_BAD(a0)
	lui	a3,tlb_umiss_cause>>16
	jal	tlb_miss		# jump to C code
	lw	a3,+tlb_umiss_cause&0x7fff(a3)
	lw	a0,MEL_EFRAME(sp)
...
VECTOR(TRAP_syscall, MINSAVED)
	...
	.set	noat
	li	AT,EML_MIN_SYSCALL	# less than minimum?
	slt	a1,AT,v0
	beq	a1,zero,syscall_native
	lw	a1,current_dispatch	# N cycles
	subu	AT,v0,AT
	beq	a1,zero,syscall_native	# min overhead: 7+N cycles
	sll	AT,2			# displacement
	lw	ra,DISP_COUNT(a1)
	addu	a1,AT			# eml vector
	sll	ra,2			# range check
	sltu	AT,ra,AT		# unsigned comparison!
	bne	AT,zero,syscall_native
...
fpa_unusable:
	lw	a1,current_pcb
	lw	a3,fpa_type
	sw	sp,MSS_SIZE+MMS_FPA_USED(a1)	# we'll use the fpa
...
1:
	...
	sw	a3,MSS_SR(a0)
	beq	a2,a3,fast_exception_end# how lucky. Take no AST.
	nop
	...
	nop
	nop
	sw	a2,MSS_SIZE+MMS_FPA_CSR(a3)
	cfc1	a2,fpa_eir
	nop
	sw	a2,MSS_SIZE+MMS_FPA_EIR(a3)
	swc1	$f31, MSS_SIZE+MMS_FPA_REGS+31*4(a3)
	swc1	$f30, MSS_SIZE+MMS_FPA_REGS+30*4(a3)
	swc1	$f29, MSS_SIZE+MMS_FPA_REGS+29*4(a3)
	swc1	$f28, MSS_SIZE+MMS_FPA_REGS+28*4(a3)
	swc1	$f27, MSS_SIZE+MMS_FPA_REGS+27*4(a3)
	swc1	$f26, MSS_SIZE+MMS_FPA_REGS+26*4(a3)
	swc1	$f25, MSS_SIZE+MMS_FPA_REGS+25*4(a3)
	swc1	$f24, MSS_SIZE+MMS_FPA_REGS+24*4(a3)
	swc1	$f23, MSS_SIZE+MMS_FPA_REGS+23*4(a3)
	swc1	$f22, MSS_SIZE+MMS_FPA_REGS+22*4(a3)
	swc1	$f21, MSS_SIZE+MMS_FPA_REGS+21*4(a3)
	swc1	$f20, MSS_SIZE+MMS_FPA_REGS+20*4(a3)
	swc1	$f19, MSS_SIZE+MMS_FPA_REGS+19*4(a3)
	swc1	$f18, MSS_SIZE+MMS_FPA_REGS+18*4(a3)
	swc1	$f17, MSS_SIZE+MMS_FPA_REGS+17*4(a3)
	swc1	$f16, MSS_SIZE+MMS_FPA_REGS+16*4(a3)
	swc1	$f15, MSS_SIZE+MMS_FPA_REGS+15*4(a3)
	swc1	$f14, MSS_SIZE+MMS_FPA_REGS+14*4(a3)
	swc1	$f13, MSS_SIZE+MMS_FPA_REGS+13*4(a3)
	swc1	$f12, MSS_SIZE+MMS_FPA_REGS+12*4(a3)
	swc1	$f11, MSS_SIZE+MMS_FPA_REGS+11*4(a3)
	swc1	$f10, MSS_SIZE+MMS_FPA_REGS+10*4(a3)
	swc1	$f9,  MSS_SIZE+MMS_FPA_REGS+ 9*4(a3)
	swc1	$f8,  MSS_SIZE+MMS_FPA_REGS+ 8*4(a3)
	swc1	$f7,  MSS_SIZE+MMS_FPA_REGS+ 7*4(a3)
	swc1	$f6,  MSS_SIZE+MMS_FPA_REGS+ 6*4(a3)
	swc1	$f5,  MSS_SIZE+MMS_FPA_REGS+ 5*4(a3)
	swc1	$f4,  MSS_SIZE+MMS_FPA_REGS+ 4*4(a3)
	swc1	$f3,  MSS_SIZE+MMS_FPA_REGS+ 3*4(a3)
	swc1	$f2,  MSS_SIZE+MMS_FPA_REGS+ 2*4(a3)
	swc1	$f1,  MSS_SIZE+MMS_FPA_REGS+ 1*4(a3)
	swc1	$f0,  MSS_SIZE+MMS_FPA_REGS+ 0*4(a3)
fpa_free:
	/*
	 *	Reload fpa state
	 */
	...
	lw	a3,current_pcb
	nop
	lwc1	$f31, MSS_SIZE+MMS_FPA_REGS+31*4(a3)
	lwc1	$f30, MSS_SIZE+MMS_FPA_REGS+30*4(a3)
	lwc1	$f29, MSS_SIZE+MMS_FPA_REGS+29*4(a3)
	lwc1	$f28, MSS_SIZE+MMS_FPA_REGS+28*4(a3)
	lwc1	$f27, MSS_SIZE+MMS_FPA_REGS+27*4(a3)
	lwc1	$f26, MSS_SIZE+MMS_FPA_REGS+26*4(a3)
	lwc1	$f25, MSS_SIZE+MMS_FPA_REGS+25*4(a3)
	lwc1	$f24, MSS_SIZE+MMS_FPA_REGS+24*4(a3)
	lwc1	$f23, MSS_SIZE+MMS_FPA_REGS+23*4(a3)
	lwc1	$f22, MSS_SIZE+MMS_FPA_REGS+22*4(a3)
	lwc1	$f21, MSS_SIZE+MMS_FPA_REGS+21*4(a3)
	lwc1	$f20, MSS_SIZE+MMS_FPA_REGS+20*4(a3)
	lwc1	$f19, MSS_SIZE+MMS_FPA_REGS+19*4(a3)
	lwc1	$f18, MSS_SIZE+MMS_FPA_REGS+18*4(a3)
	lwc1	$f17, MSS_SIZE+MMS_FPA_REGS+17*4(a3)
	lwc1	$f16, MSS_SIZE+MMS_FPA_REGS+16*4(a3)
	lwc1	$f15, MSS_SIZE+MMS_FPA_REGS+15*4(a3)
	lwc1	$f14, MSS_SIZE+MMS_FPA_REGS+14*4(a3)
	lwc1	$f13, MSS_SIZE+MMS_FPA_REGS+13*4(a3)
	lwc1	$f12, MSS_SIZE+MMS_FPA_REGS+12*4(a3)
	lwc1	$f11, MSS_SIZE+MMS_FPA_REGS+11*4(a3)
	lwc1	$f10, MSS_SIZE+MMS_FPA_REGS+10*4(a3)
	lwc1	$f9,  MSS_SIZE+MMS_FPA_REGS+ 9*4(a3)
	lwc1	$f8,  MSS_SIZE+MMS_FPA_REGS+ 8*4(a3)
	lwc1	$f7,  MSS_SIZE+MMS_FPA_REGS+ 7*4(a3)
	lwc1	$f6,  MSS_SIZE+MMS_FPA_REGS+ 6*4(a3)
	lwc1	$f5,  MSS_SIZE+MMS_FPA_REGS+ 5*4(a3)
	lwc1	$f4,  MSS_SIZE+MMS_FPA_REGS+ 4*4(a3)
	lwc1	$f3,  MSS_SIZE+MMS_FPA_REGS+ 3*4(a3)
	lwc1	$f2,  MSS_SIZE+MMS_FPA_REGS+ 2*4(a3)
	lwc1	$f1,  MSS_SIZE+MMS_FPA_REGS+ 1*4(a3)
	lwc1	$f0,  MSS_SIZE+MMS_FPA_REGS+ 0*4(a3)
	ctc1	zero,fpa_csr		# clear status [Kane]
	lw	a1,MSS_SIZE+MMS_FPA_EIR(a3)
	nop
	ctc1	a1,fpa_eir		# restore trap status
	lw	a1,MSS_SIZE+MMS_FPA_CSR(a3)
	b	fast_exception_end	# take no AST
...
LEAF(wrclr_buserror)
	...
	la	a2, WRCLR_BUSERR
	la	ra, TRAP_generic	# no need to return
	j	wbflush
	sw	zero, 0(a2)
	END(wrclr_buserror)
