...
#define	peek_inst(addr)		db_get_value(addr,4,0)
...
db_disasm(loc, altfmt)
	...
	boolean_t	altfmt;
{
	loc += mips_print_instruction(loc, peek_inst(loc), altfmt);
	return (loc);
}

mips_print_instruction(iadr, i, showregs)
	unsigned iadr;
	mips_instruction i;
{
	char *s;
...
branch_displacement:
		...
		db_printsym(iadr+4+(signed_immediate<<2), DB_STGY_PROC);
		print_next = 1;
		break;
	case op_cop0:
	...
	case op_jal:
		...
		db_printsym(unsigned_immediate, DB_STGY_PROC);
		print_next = 1;
		break;
...
loadstore:
	...
	if (print_next) {
		db_printf("\n\t");
		mips_print_instruction(iadr+4, peek_inst(iadr+4), showregs);
		ibytes += 4;
	}
	return(ibytes);
}
