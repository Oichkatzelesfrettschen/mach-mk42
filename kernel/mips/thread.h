...
struct mips_machine_state {
	/*
	 * Floating point state (coprocessor 1)
	 */
	unsigned	fpa_registers[32];
	unsigned	fpa_csr;	/* control and status reg */
	unsigned	fpa_eir;	/* exception instruction reg */
	unsigned	fpa_used;	/* has thread ever used the fp */
	/*
	 * Single stepping
	 */
	short	sstepping;	/* non-zero if single stepping */
	short	ss_count;	/* no. of breakpoints installed */
	struct	breakpoint {
...
void		pcb_init(), pcb_terminate(), pcb_sync();
void		thread_start();
