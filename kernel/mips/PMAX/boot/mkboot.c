	...
	char	*out, *in, *lab;
{
	...
	union {
		...
		struct {
			scsi_dec_boot0_t	b;
			char	pad[BSD_PARTITION_BYTE_OFFSET-sizeof(scsi_dec_boot0_t)];
			scsi_bsd_label_t	l;
		} info;
	...
	scsi_bsd_label_t	*l;
{
	...
	end = (unsigned short *)&l->partitions[l->npartitions];
...
#	define LSIZ	100
	 ...
	 * Label proper 
	 */
	l->magic = l->magic2 = BSD_PARTITION_MAGIC;
	l->type = LAB_DTYPE_SCSI;

	if (!updating || confirm(would_you, "Disk type")) {
	    n = get_confirmed(
			      "Disk type name",
			      updating ? l->typename : "rz56",
			      "string_16",
			      answ, 16);
	    bcopy(answ, l->typename, n);

	    sprintf(deflt, "%d", l->subtype);
	    n = get_confirmed(
	...
	if (!updating || confirm(would_you, "Pack label")) {
	    n = get_confirmed(
			      "Disk pack name",
			      updating ? l->packname : "mydisk",
			      "string_16",
			      answ, 16);
	    bcopy(answ, l->packname, n);
	}
	...
	if (!updating || confirm(would_you, "Disk geometry parameters")) {
	    ...
	    sprintf(deflt, "%d", l->nsectors);
	    n = get_confirmed(
			      "Sectors per track",
			      updating ? deflt : "54",
			      "decimal u_int_32",
			      answ, LSIZ);
	    l->nsectors = atoi(answ);

	    sprintf(deflt, "%d", l->ntracks);
	    n = get_confirmed(
			      "Tracks per cylinder",
			      updating ? deflt : "15",
			      "decimal u_int_32",
	    ...
	    sprintf(deflt, "%d", l->ncylinders);
	    n = get_confirmed(
			      "Number of cylinders",
			      updating ? deflt : "1632",
			      "decimal u_int_32",
			      answ, LSIZ);
	    l->ncylinders = atoi(answ);

	    l->secpercyl = l->nsectors * l->ntracks;
	    l->secperunit = l->secpercyl * l->ncylinders;

	    /*
	     * Spares
	     */
	    sprintf(deflt, "%d", l->sparespertrack);
	    n = get_confirmed(
			      "Spare sectors per track",
			      updating ? deflt : "1",
			      "decimal u_short_16",
			      answ, LSIZ);
	    l->sparespertrack = atoi(answ);

	    sprintf(deflt, "%d", l->sparespercyl);
	    n = get_confirmed(
			      "Spare sectors per cylinder",
			      updating ? deflt : "1",
			      "decimal u_short_16",
			      answ, LSIZ);
	    l->sparespercyl = atoi(answ);

	    sprintf(deflt, "%d", l->acylinders);
	    n = get_confirmed(
			      "Spare cylinders",
			      deflt,
			      "decimal u_int_32",
			      answ, LSIZ);
	    l->acylinders = atoi(answ);
	...
	if (!updating || confirm(would_you, "Hardware characteristics")) {
	    ...
	    sprintf(deflt, "%d", l->interleave);
	    n = get_confirmed(
			      "Interleave factor",
			      updating ? deflt : "1",
			      "decimal u_short_16",
			      answ, LSIZ);
	    l->interleave = atoi(answ);

	    sprintf(deflt, "%d", l->trackskew);
	    n = get_confirmed(
			      "Track skew",
			      deflt,
			      "decimal u_short_16",
			      answ, LSIZ);
	    l->trackskew = atoi(answ);

	    sprintf(deflt, "%d", l->cylskew);
	    n = get_confirmed(
			      "Cylinder skew",
			      deflt,
			      "decimal u_short_16",
	    ...
	    sprintf(deflt, "%d", l->headswitch);
	    n = get_confirmed(
			      "Head switch time (usecs)",
			      deflt,
			      "decimal u_int_32",
			      answ, LSIZ);
	    l->headswitch = atoi(answ);

	    sprintf(deflt, "%d", l->trkseek);
	    n = get_confirmed(
	...
	bzero(l->drivedata, NDDATA * sizeof(int));
	bzero(l->spare, NSPARE * sizeof(int));
	...
	if (!updating || confirm(would_you, "Partitioning")) {
	    ...
	    sprintf(deflt, "%d", l->npartitions);
	    n = get_confirmed(
			      "Number of partitions",
			      updating ? deflt : "8",
			      "decimal u_short_16",
			      answ, LSIZ);
	    l->npartitions = atoi(answ);

	    sprintf(deflt, "%x", l->bbsize);
	    n = get_confirmed(
		...
		static char    *p = "Number of sectors in partition X";
	    	...
	    	n = strlen(p) - 1;
	    	p[n] = 'a' + i;
		sprintf(deflt, "%d", l->partitions[i].size);
	    	n = get_confirmed(
			    	  p,
				  deflt,
			    	  "decimal u_int_32",
			    	  answ, LSIZ);
	    	l->partitions[i].size = atoi(answ);

		sprintf(deflt, "%d", l->partitions[i].offset);
	    	n = get_confirmed(
			         "Starting sector of partition",
				 deflt,
			    	  "decimal u_int_32",
			    	  answ, LSIZ);
	    	l->partitions[i].offset = atoi(answ);

		printf("File system type on partition: BSD->7, unused->0 ");
		sprintf(deflt, "%d", l->partitions[i].fstype);
	    	n = get_confirmed(
			    	  "as per rz_label.h",
				  updating ? deflt : "7",
			    	  "decimal u_int_8",
			    	  answ, LSIZ);
	    	l->partitions[i].fstype = atoi(answ);

		sprintf(deflt, "%d", l->partitions[i].fsize);
	    	n = get_confirmed(
			          "Filesystem fragment size",
				  updating ? deflt : "1024",
			    	  "decimal u_int_32",
			    	  answ, LSIZ);
	    	l->partitions[i].fsize = atoi(answ);

		sprintf(deflt, "%d", l->partitions[i].frag);
	    	n = get_confirmed(
			    	  "Fragments per block",
				  updating ? deflt : "8",
			    	  "decimal u_int_8",
			    	  answ, LSIZ);
		l->partitions[i].frag = atoi(answ);

		sprintf(deflt, "%d", l->partitions[i].cpg);
		n = get_confirmed(
			          "Cylinders per cylinder-group",
				  updating ? deflt : "16",
			    	  "decimal u_int_16",
				  answ, LSIZ);
	    	l->partitions[i].cpg = atoi(answ);
	    }
	}
 ...
 */
print_label(l)
	register scsi_bsd_label_t	*l;
{
	register int			i;
	static char			*typenames[LAB_FS_BSDFFS+1] = {
		"-- unused --", "swap space", "Sixth Edition",
		"Seventh Edition", "System-V", "Seventh Edition (1k blocks)",
		"Eigth Edition", "4.2BSD"
	};

	if (l->magic != BSD_PARTITION_MAGIC ||
	    l->magic2 != BSD_PARTITION_MAGIC ||
	    label_sum(l) != 0) {
	...
	for (i = 0; i < l->npartitions; i++) {
		printf("\t%c\t%d\t%d\t%d\t%d\t%d\t",
			'a' + i, l->partitions[i].size, l->partitions[i].offset,
			l->partitions[i].fsize,
			l->partitions[i].fsize * l->partitions[i].frag,
			l->partitions[i].cpg);
		if (l->partitions[i].fstype <= LAB_FS_BSDFFS)
			printf("%s\n", typenames[l->partitions[i].fstype]);
		else
			printf("?type x%x?\n", l->partitions[i].fstype);
	}
}
