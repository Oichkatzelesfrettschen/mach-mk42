...
execit:
	...
	(*((int (*) ()) hdr.a.entry)) (argc, argv, envp, 1);
}
	...
	struct dev_t	*dev;
{
	...
	l = (scsi_bsd_label_t*)((char*)l + BSD_PARTITION_BYTE_OFFSET);
	if (l->magic == BSD_PARTITION_MAGIC) {
		...
		if (c < '0' || c > '9')
			return 0;
		dev->first_block = l->partitions[c - '0'].offset;
		dev->last_block  = dev->first_block + l->partitions[c - '0'].size;
	}
