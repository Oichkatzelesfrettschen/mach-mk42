...
struct drivers_map {
	...
	{ "KN02    ",	"dz"},		/* system board, care for DZ */
	{ "PMAD-AA ",	"se"},		/* Ether */
	{ "PMAZ-AA ",	"asc"},		/* SCSI */
...
boolean_t
tc_identify_option_from_rom( addr, slot, complain)
	tc_rommap_t	*addr;
	tc_option_t	*slot;
	boolean_t	complain;
{
...
#	define		TC_N_OFFSETS	2
	...
	for (i = 0; i < TC_N_OFFSETS; i++) {
		...
		if (tc_identify_option_from_rom(addr + tc_offset_rom[i],
...
unsigned int	tc_slot_phys_base[KN02_TC_NSLOTS] = {
...
void (*tc_slot_hand_fill)(/* tc_option_t * */) = 0;
...
tc_find_all_options()
{
	...
	for (i = KN02_TC_MAX; i >= KN02_TC_MIN;) {
	...
	for (i = 0, sl = tc_slot_info; i < KN02_TC_NSLOTS; i++, sl++) {
		...
		if (!found) {
			dprintf("%s%s%s\n",
				"Fatal: device driver '", sl->driver_name,
				"' missing from autoconf tables");
			prom_restart();
		}
	}
}
...
tc_autoconf()
{
	 ...
	 * Interrupts off
	 */
	for (i = 0; i < KN02_TC_NSLOTS; i++)
		tc_enable_interrupt(i, FALSE);
	...
	for (i = 0, sl = tc_slot_info; i < KN02_TC_NSLOTS; i++, sl++) {
		 ...
		 */
		tc_enable_interrupt(i, (sl->driver->flags & BUS_INTR_DISABLED) == 0);
	}
}

/*
 * These might need yet another switch table.  Someday.
 */
...
tc_enable_interrupt(slotno, on)
	...
	boolean_t	on;
{
	/*XXX*/kn02_enable_interrupt(slotno, on);
}
