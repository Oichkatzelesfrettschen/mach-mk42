...
static	caddr_t SEstd[NSE] = { 0 };
static	struct bus_device *SEinfo[NSE];
...
#define SE_REG_PHYS(unit)	K1SEG_TO_PHYS(REGBASE(unit)+se_sw->regspace)
#define SE_REG_SIZE		PAGE_SIZE

#define SE_BUF_PHYS(unit)	K1SEG_TO_PHYS(REGBASE(unit)+se_sw->bufspace)
#define SE_BUF_SIZE		(6128*1024)

#define SE_ADR_PHYS(unit)	K1SEG_TO_PHYS(REGBASE(unit)+se_sw->romspace)
	...
	register struct bus_device *ui;
{
	 ...
	 */
	addr = kmem_alloc(kernel_map, PAGE_SIZE);	/* kseg2 */
	addr = pmap_extract(pmap_kernel(), addr);	/* phys */
	info = (mapped_ether_info_t) (PHYS_TO_K0SEG(addr));
	...
	info->buffer_physaddr = (REGBASE(unit) + se_sw->bufspace)&0xffffff;
...
extern boolean_t se_use_mapped_interface[NSE];
	...
	io_req_t ior;
{
	...
	/*
	 * Make thread unswappable (for handoff scheduling) 
	 */
...
SE_close(dev, flag)
{
	...
	/*
	 * Make thread swappable again
	 */
	if (thread = self->user_thread) {
		self->user_thread = 0;
		thread_swappable(thread, TRUE);
	}
...
/*verify 3max: SE0_BUF_SIZE + SE0_REG_SIZE + SE0_ADR_SIZE + PAGE_SIZE;*/
...
#define S3	(S2+SE_BUF_SIZE)
	...
	if (off < S0) {
		addr = K0SEG_TO_PHYS (SE_statii[unit].info);
	} else if (off < S1) {
