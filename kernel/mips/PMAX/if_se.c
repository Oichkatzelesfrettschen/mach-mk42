...
static caddr_t se_std[NSE] = { 0 };
static struct bus_device *se_info[NSE];
...
struct	se_softc {
	...
	volatile unsigned	lnbuf;
	volatile unsigned	lnrom;
...
se_softc_t	se_softc[NSE];		/* quick access */
...
/* pmax */
	{ 0xb8000000, 0xb9000000, 0xbd000000, 8, 16, 64*1024, 
	  copyin_gap16, copyout_gap16, bzero_gap16, se_Hmem_gap16},
/* 5400 */
	{ 0xb0084400, 0xb0120000, 0xb0084200, 0, 0, 64*1024 /*?*/, 
	  bcopy, bcopy, bzero, se_Hmem_nogap},
/* 3max */
	{ PMAD_OFFSET_LANCE, PMAD_OFFSET_RAM, PMAD_OFFSET_ROM,
	  16, 0, PMAD_RAM_SIZE,
	  bcopy, bcopy, bzero, se_Hmem_nogap},
};
...
#define Lmem(lna)	(volatile char*)((unsigned)lna | (sc->lnbuf&0xffffff))
	...
	register struct bus_device *ui;
{
	...
	sc->lnbuf = (volatile unsigned) (reg + se_sw->bufspace);
	sc->lnrom = (volatile unsigned) (reg + se_sw->romspace);
...
se_write_reg(regptr, val, result, regname)
	register volatile short *regptr;
	register		 val, result;
	char			*regname;
{
	register int    i = 0;
	...
	register se_softc_t sc;
{
	 ...
	 */
	(se_sw->copyout)(&init_block, Hmem(sc->lninit_block), sizeof(init_block));
	wbflush();
}
	...
	u_char	eaddr[6];
{
	 ...
	 */
	(se_sw->copyin) (Hmem(sc->lninit_block), &init_block, sizeof(init_block));
	...
	int unit;
{
	...
	(se_sw->copyin) (Hmem(sc->lninit_block), &mode, sizeof(mode));
	...
	if (ifp->if_flags & IFF_LOOPBACK
		...
		mode &= ~LN_MODE_INTL;
		mode |= LN_MODE_LOOP;
		(se_sw->copyout) (&mode, Hmem(sc->lninit_block), sizeof(mode));
		se_restart(ifp);
		se_init(ifp->if_unit);
		splx(s);
		return;
...
int se_use_mapped_interface[NSE];
...
se_intr(unit,spllevel)
{
	...
	if (se_open_state < 2) { /* Stray, or not open for business */
		rdp = (sc ? sc->lnregs : (se_reg_t)(se_sw->regspace));
		*rdp |= LN_CSR0_STOP;
		wbflush();
		return;
	}
		...
		if (csr & LN_CSR0_MISS) {
			...
			if (se_verbose) {
				...
				for (index = 0; index < NRCV; index++) {
					(se_sw->copyin)(sc->lnrring[index], &r, sizeof(r));
	...
	int unit;
{
	...
	while ((sc->xmt_complete != sc->xmt_last) && (sc->xmt_count > 0)) {
		...
		(se_sw->copyin) (sc->lntring[index], &r, sizeof(r));
		status = r.status;
	...
	int unit;
{
	...
	for (;; sc->rcv_last = ++index & (NRCV - 1)) {
		 ...
		 */
read_descriptor:
		(se_sw->copyin) (sc->lnrring[sc->rcv_last], &r, sizeof(r));
		status = r.status;
		...
		while (((status1 & (LN_RSTATE_ERR | LN_RSTATE_OWN | LN_RSTATE_ENP)) == 0) &&
			...
			(se_sw->copyin) (sc->lnrring[index], &r1, sizeof(r1));
			status1 = r1.status;
		}
		...
		if ((status & ~LN_RSTATE_STP) == LN_RSTATE_ENP) {
			...
			(se_sw->copyout) (&r, sc->lnrring[first], sizeof(r));
			continue;
		}
		/*
	...
	int	unit;
{
	...
	for (index = sc->xmt_last;
		...
		(se_sw->copyout)(&r, sc->lntring[index], sizeof(r));
		wbflush();
	...
	struct packet_header 	 *pkt;
{
	 ...
	 */
	(se_sw->copyin) (rbuf, hdr, sizeof(struct ether_header));

	nbytes -= sizeof(struct ether_header);
	rbuf += (se_sw->mapaddr) (sizeof(struct ether_header));
	...
	(se_sw->copyin) (rbuf, pkt + 1, nbytes);
}
	...
	register se_desc_t	lndesc;
{
	...
	(se_sw->copyin) (lndesc, &desc, sizeof(desc));
	desc.desc4.bits = 0;
	desc.status     = val;
	(se_sw->copyout) (&desc, lndesc, sizeof(desc));
	wbflush();
}
	...
	int		size;
{
	...
	/*
	 * Start out on the first longword boundary 
	 */
	if (sc->lnsbrk & 7)
		sc->lnsbrk = (sc->lnsbrk + 8) & ~7;
