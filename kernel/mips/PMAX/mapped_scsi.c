...
#include <mips/PMAX/mapped_scsi.h>
...
#define SII_REG_PHYS(self)	K1SEG_TO_PHYS(self->registers.any)
...
#define ASC_REG_PHYS(self)	K1SEG_TO_PHYS(self->registers.any)
	...
	register struct bus_device 	*ui;
{
	...
	info = (mapped_scsi_info_t) (PHYS_TO_K0SEG(addr));
	...
	dev_t		dev;
{
	...
	if (off < SCSI_INFO_SIZE) {
		addr = K0SEG_TO_PHYS (self->info) + off;
		ret = D_SUCCESS;
	} else
