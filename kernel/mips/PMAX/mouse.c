...
mouse_softc_t	mouse_softc_data[NLK], *mouse_softc[NLK];
...
mouse_attach(unit)
{
	...
	s = spltty();
	dz_putc(unit, DZ_LINE_MOUSE, MOUSE_SELFTEST);
	delay(100000);	/* spec says less than 500 msecs */
	messg[0] = dz_getc(unit, DZ_LINE_MOUSE, TRUE, TRUE);
	messg[1] = dz_getc(unit, DZ_LINE_MOUSE, TRUE, TRUE);
	messg[2] = dz_getc(unit, DZ_LINE_MOUSE, TRUE, TRUE);
	messg[3] = dz_getc(unit, DZ_LINE_MOUSE, TRUE, TRUE);
	dz_putc(unit, DZ_LINE_MOUSE, MOUSE_INCREMENTAL);
	splx(s);
