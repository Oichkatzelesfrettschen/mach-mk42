...
static struct tty 	dz_tty_data[NDZ*NDZLINE];
struct tty		*dz_tty[NDZ*NDZLINE];	/* quick access */
	...
	io_req_t ior;
{
	...
	if (unit >= NDZLINE*NDZ)
		return (ENXIO);
	tp = dz_tty[unit];
	tp->t_addr = (caddr_t)&dz_pdma[unit/NDZLINE];
	tp->t_oproc = dz_start;
	tp->t_stop = dz_stop;
