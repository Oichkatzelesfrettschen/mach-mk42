...
#include <sys/types.h>
#include <sys/ioctl.h>
#include <mips/PMAX/lk201.h>
#include <mips/PMAX/dz_7085.h>
...
softc_t 	lk201_softc_data[NLK];
softc		lk201_softc[NLK];
...
lk201_reset(unit)
{
	...
	for (i = 0; i < sizeof(lk201_reset_string); i++)
		dz_putc(unit, DZ_LINE_KEYBOARD, lk201_reset_string[i]);
	/* zero any state associated with previous keypresses */
	bzero(&lk201_softc_data[unit], sizeof(lk201_softc_data[unit]));
}
...
lk201_selftest(unit)
{
	...
	s = spltty();
	dz_putc(unit, DZ_LINE_KEYBOARD, LK_CMD_REQ_ID);
	delay(10000);/* arbitrary */
	messg[0] = dz_getc(unit, DZ_LINE_KEYBOARD, TRUE, TRUE);
	messg[1] = dz_getc(unit, DZ_LINE_KEYBOARD, TRUE, TRUE);
	splx(s);

	printf("( lk201 id %x.%x", unit, messg[0], messg[1]);

	s = spltty();
	dz_putc(unit, DZ_LINE_KEYBOARD, LK_CMD_POWER_UP);
	delay(80000);/* spec says 70 msecs or less */
	messg[0] = dz_getc(unit, DZ_LINE_KEYBOARD, TRUE, TRUE);
	messg[1] = dz_getc(unit, DZ_LINE_KEYBOARD, TRUE, TRUE);
	messg[2] = dz_getc(unit, DZ_LINE_KEYBOARD, TRUE, TRUE);
	messg[3] = dz_getc(unit, DZ_LINE_KEYBOARD, TRUE, TRUE);
	splx(s);

	printf(", self-test ", unit);
...
{
	dz_putc(unit, DZ_LINE_KEYBOARD, LK_CMD_BELL);
}

/*
 *	Here is your LED toy, Bob
 */
lk201_lights(unit, on)
{
	dz_putc(unit, DZ_LINE_KEYBOARD, LK_CMD_LEDS_OFF);
	dz_putc(unit, DZ_LINE_KEYBOARD, LK_PARAM_LED_MASK(0xf));
	if (on < 16 && on > 0) {
		dz_putc(unit, DZ_LINE_KEYBOARD, LK_CMD_LEDS_ON);
		dz_putc(unit, DZ_LINE_KEYBOARD, LK_PARAM_LED_MASK(on));
	}
}
...
lk201_led(unit)
{
	...
	if (leds->led_interval) {		/* leds are on */
	    if (leds->led_light_count <= 0) {	/* hit this lights */
	        ...
	        dz_putc( unit, DZ_LINE_KEYBOARD, LK_CMD_LEDS_OFF);
	        dz_putc( unit, DZ_LINE_KEYBOARD, LK_PARAM_LED_MASK(leds->led_lights));
 ...
 */
lk201_set_status(unit, flavor, status, status_count)
	int		unit;
	int		flavor;
	dev_status_t	status;
	unsigned int	status_count;
{
	...
	switch( flavor ) {
		...
		dz_putc(unit, DZ_LINE_KEYBOARD, cmd->command);
		if (cnt > 0)
			dz_putc(unit, DZ_LINE_KEYBOARD, cmd->params[0]);
		if (cnt > 1)
			dz_putc(unit, DZ_LINE_KEYBOARD, cmd->params[1]);
		return D_SUCCESS;
	    }
...
lk201_translation( key, c, tabcode )
	char c;
{
	unsigned char *table;
...
lk201_rint(unit, data, handle_shift, from_kernel)
	unsigned short	data;
	...
	unsigned short	data;
{
	...
	case LK_LOCK:
		...
		if (kbd->kbd_lock)
			...
			dz_putc(unit, DZ_LINE_KEYBOARD, LK_CMD_LEDS_OFF);
		dz_putc(unit, DZ_LINE_KEYBOARD, LK_PARAM_LED_MASK(0x4));
		return 0;
