...
/*
 * Register layout, padding included
 */

typedef struct {
	volatile unsigned short	dz_csr;		/* Control and Status */
	char							pad0[6];
	volatile unsigned short	dz_rbuf;	/* Rcv buffer */
#	define 			dz_lpr dz_rbuf	/* Line parameters */
	char							pad1[6];
	volatile unsigned short	dz_tcr;		/* Xmt control */
	char							pad2[6];
	volatile unsigned short	dz_tbuf;	/* Xmt buffer */
#	define 			dz_msr dz_tbuf	/* Modem status */
	char							pad3[6];
} dz_regmap_t;
