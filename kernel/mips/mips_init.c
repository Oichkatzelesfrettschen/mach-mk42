	...
	char *argv[];
{
	...
	cons_enable();		/* initialize console device: MUST be there */
	...
	vm_offset_t from, to;
{
	...
	bitmap = (int*)string_to_int(prom_getenv("bitmap"));	/* brrrr */
	len = string_to_int(prom_getenv("bitmaplen"));
	...
	for (i = 2; i < len; i++, bitmap++) {
		if (*bitmap == 0xffffffff) {
			...
			/* last word, get partials and out */
			for (i = 0; i < 32; i++)
				if (*bitmap & (1 << i))
					size++;
			break;
	...
	/*
	 * Zero out memory, appears to be still needed ? [init 7 -m]
	 */
	if (size < to) to = size;
...
#ifdef	PMAX
	/*
	 * I need a page aligned page to map shared screen control info.
	 * And I need it now.  I will do a cons_enable() next.
	 */
	{
		extern vm_size_t	screen_memory_alloc();
		vm_size_t		needed;

		needed = screen_memory_alloc(PHYS_TO_K0SEG(avail_start));
		avail_start += round_page(needed);
	}
#endif	PMAX
	if (zalloc_physical) {
		extern vm_offset_t zalloc_next_space, zalloc_end_of_space;
		zalloc_next_space = PHYS_TO_K0SEG(avail_start);
		avail_start += round_page(zalloc_physical);
		zalloc_end_of_space = PHYS_TO_K0SEG(avail_start);
	}
}
