 ...
 * 	Added MACH_CLPORT support (hack in ikm_free).
...
/*
 *	The size of the kernel message buffers that will be stored in
 *	ith_saved.  In general this field is only used in the thread
 *	itself, so it doesn't need a lock.  An exception: thread_collect_scan
 *	frees saved messages, but only when the thread can't be executing.
 *
