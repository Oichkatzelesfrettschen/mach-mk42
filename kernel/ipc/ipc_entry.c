	...
	ipc_space_t space;
{
	...
	do {
		...
		is_write_lock(space);
		if (!space->is_active ||
		    (space->is_table_next != its)) {
			/*
			 *	The space died while it was unlocked, or
			 *	somebody else slipped in and grew the table.
			 */
		...
		assert(osize == space->is_table_size);
		...
		/*
		 *	Clear the new table.  Zero the new part,
		 *	but in the old part, just zero index fields.
		 *	The other fields in the old part get copied.
		 */
		 ...
		 *	Put entries into the reverse hash table.
		 */

		for (i = 0; i < osize; i++) {
			ipc_entry_t oentry = &otable[i];
			ipc_entry_t entry = &table[i];
			ipc_entry_bits_t bits = oentry->ie_bits;
			ipc_object_t obj = oentry->ie_object;

			entry->ie_bits = bits;
			entry->ie_object = obj;
			entry->ie_request = oentry->ie_request;

			if (IE_BITS_TYPE(bits) == MACH_PORT_TYPE_SEND)
				ipc_hash_local_insert(space, obj, i, entry);
		}

		/*
		 *	We've sucked everything out of the old table.
		 */
