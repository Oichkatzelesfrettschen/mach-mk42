	...
	mach_msg_timeout_t timeout;
{
	...
	for (;;) {
		...
		if (option & MACH_SEND_TIMEOUT) {
			...
			ipc_thread_will_wait_with_timeout(self, timeout);
		} else
			ipc_thread_will_wait(self);

		ipc_thread_enqueue(&port->ip_blocked, self);
		self->ith_state = MACH_SEND_IN_PROGRESS;

	 	ip_unlock(port);
		ipc_thread_block(self, (void (*)()) 0);
		ip_lock(port);
	...
	ipc_kmsg_t *kmsgp;
{
	...
	for (;;) {
		...
		imq_unlock(mqueue);
		ipc_thread_block(self, continuation);
	after_ipc_thread_block:
		imq_lock(mqueue);
