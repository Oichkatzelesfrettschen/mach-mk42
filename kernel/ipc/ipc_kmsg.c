	...
	while (saddr < eaddr) {
		...
		if (is_inline) {
			...
			if (length == 0) {
				...
				kr = vm_map_copyout(map, &addr,
						    (vm_map_copy_t) data);
				if (kr != KERN_SUCCESS) {
					/* copy is already discarded */
	...
	vm_map_t map;
{
	...
	kmsg->ikm_header.msgh_kind = (mach_msg_kind_t) msg.msg_type;
	...
	vm_map_t map;
{
	...
	msg.msg_type = (int) kmsg->ikm_header.msgh_kind;
