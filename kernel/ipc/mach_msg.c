 ...
 * 	Added MACH_CLPORT support: always ipc_mqueue_send() to a remote port.
	...
	mach_port_t notify;
{
	...
	if (mr != MACH_MSG_SUCCESS) {
		if ((mr &~ MACH_MSG_MASK) == MACH_RCV_HEADER_ERROR) {
			ipc_kmsg_copyout_dest(kmsg, space);
			(void) ipc_kmsg_put(msg, kmsg, sizeof *msg);
		} else if ((mr &~ MACH_MSG_MASK) == MACH_RCV_BODY_ERROR)
	...
	mach_port_t notify;
{
	...
	if (option == (MACH_SEND_MSG|MACH_RCV_MSG)) {
		...
		if (((kmsg = self->ith_saved) == IKM_NULL) ||
		    (send_size > IKM_SAVED_MSG_SIZE) ||
		    (send_size < sizeof(mach_msg_header_t)) ||
		    (send_size & 3) ||
		    copyinmsg((char *) msg, (char *) &kmsg->ikm_header,
			      send_size))
			goto slow_get;

		self->ith_saved = IKM_NULL;
		 ...
		 *	We have hope of hand-off, so start preparations.
		 */

		self->swap_func = mach_msg_continue;
		self->ith_msg = msg;
		self->ith_rcv_size = rcv_size;
		...
		if ((receiver->swap_func == (void (*)()) mach_msg_continue) &&
		    ipc_thread_switch(self, receiver)) {
		 ...
		 *	we have kmsg.  Must still check against rcv_size
		 *	and do ipc_kmsg_copyout/ipc_kmsg_put.
		 */
