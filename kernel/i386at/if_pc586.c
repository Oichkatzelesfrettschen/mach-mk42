...
struct isa_dev	*dev;
{
	...
	if ((b_prom = (volatile char *)kmem_alloc_pageable(kernel_map, len)) == NULL) {
		printf("pc%d: can not allocate memory for prom.\n", unit);
		return 0;
	}
	if ((b_sram = (volatile char *)kmem_alloc_pageable(kernel_map, sram_len)) == NULL) {
	...
	struct isa_dev	*dev;
{
	...
	u_char		*addr_p;
	u_short		*b_addr;
	u_char		unit = (u_char)dev->dev_unit;	
	pc_softc_t	*sp = &pc_softc[unit];
	volatile scb_t	*scb_p;

	take_dev_irq(dev);
	printf("pc%d:  port = %x, spl = %d, pic = %d. ",
