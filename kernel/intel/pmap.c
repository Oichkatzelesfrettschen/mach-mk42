	...
	vm_offset_t	*v_end;		/* OUT */
{
	...
	*v_end = phystokv(*av_end);
	...
	if (pte_to_pa(*pde) == 0) {
	    ...
	    pte += ptenum(*v_avail);		/* point to pte that
						   maps first avail VA */
...
#else
	for (va = *v_avail; va < *v_end; va += INTEL_PGBYTES) {
#endif
	    if (pte >= ptend) {
	...
	if (pte_to_pa(*ppde) == 0) {
		/* This pte has not been allocated */
		ppte = (pt_entry_t *)kvtophys(*v_avail);
		ptend = ppte + NPTES;
		*v_avail = phystokv((vm_offset_t)ptend);
	...
	WRITE_PTE(ppte + ptenum(FIFO_ADDR + XEOD_OFF),
		  pa_to_pte(FIFO_ADDR_PH + XEOD_OFF)
		| INTEL_PTE_VALID | INTEL_PTE_WRITE | INTEL_PTE_NCACHE);
 ...
 */
	*(int *)&template = 0;
	*v_end += morevm;
	for (tva = va; tva < *v_end; tva += INTEL_PGBYTES) {
	    if (pte >= ptend) {
#if i860
		pte = (pt_entry_t *)kvtophys(*v_avail);
#else
		pte = (pt_entry_t *)*v_avail;
#endif
		ptend = pte + NPTES;
	...
	vm_offset_t	phys_start, phys_end;
{
	...
	s = round_page(s);
/*	vm_first_phys = phys_end;	/* hack to not use pvlist */
	...
	vm_offset_t	p;
{
	...
	if (p < first_addr) {
		ret = FALSE;
	} else if (p < (vm_offset_t)(cnvmem * 1024)) {	/* 0x006 - */
		ret = TRUE;
	} else if (p < (vm_offset_t)first_avail) {	/* 0x0a0 - */
		ret = FALSE;
	} else if (p < last_addr) {			/* 0x17c - */
		ret = TRUE;
	} else {
		ret = FALSE;
	}
 ...
 */
	...
	if ((p->dirbase = (pt_entry_t *)kmem_alloc(kernel_map, INTEL_PGBYTES))
	    == PT_ENTRY_NULL)
	...
	register vm_offset_t	v;
{
	 ...
	 */
	pa = m->phys_addr;
	vm_page_init(m, pmap_object, pa, pa);
	vm_page_lock_queues();
	vm_page_wire(m);
	vm_page_unlock_queues();
	...
	int op;
{
	...
	while ( base < end ) {
		...
		if ( pte == PT_ENTRY_NULL ) return( 0 );
		if (!(*pte & INTEL_PTE_VALID)) return( 0 );
		base += PAGE_SIZE;
	}   
	return( 1 );
}
