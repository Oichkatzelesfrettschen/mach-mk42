...
struct db_variable db_regs[] = {
	"cs",	(int *)&ddb_regs.cs,  FCN_NULL,
	"ds",	(int *)&ddb_regs.ds,  FCN_NULL,
	"es",	(int *)&ddb_regs.es,  FCN_NULL,
	"fs",	(int *)&ddb_regs.fs,  FCN_NULL,
	"gs",	(int *)&ddb_regs.gs,  FCN_NULL,
	"ss",	(int *)&ddb_regs.ss,  FCN_NULL,
	"eax",	(int *)&ddb_regs.eax, FCN_NULL,
	"ecx",	(int *)&ddb_regs.ecx, FCN_NULL,
	"edx",	(int *)&ddb_regs.edx, FCN_NULL,
	"ebx",	(int *)&ddb_regs.ebx, FCN_NULL,
	"esp",	(int *)&ddb_regs.uesp,FCN_NULL,
	"ebp",	(int *)&ddb_regs.ebp, FCN_NULL,
	"esi",	(int *)&ddb_regs.esi, FCN_NULL,
	"edi",	(int *)&ddb_regs.edi, FCN_NULL,
	"eip",	(int *)&ddb_regs.eip, FCN_NULL,
	"efl",	(int *)&ddb_regs.efl, FCN_NULL,
};
	...
	int is_trap;			/* in */
{
	...
	if (is_trap == 0) {
	    ...
	    *fp = (struct i386_frame *)
	    ...
	    if (saved_regs->trapno < TRAP_TYPES) {
		db_printf("--- %s trap ---\n",
	    ...
	    db_printsym(saved_regs->eip, DB_STGY_XTRN);
	    db_printf(":\n");
	...
	char		*modif;
{
	...
	if (!have_addr) {
	    ...
	    kstack= db_get_value((int)&th->kernel_stack, 4, FALSE);
	    if (kstack == 0) {
		register struct i386_saved_state *iss = &t_pcb->iss;
		frame = (struct i386_frame *)
		    db_get_value((int)&iss->ebp, 4, FALSE);
		callpc = (db_addr_t)db_get_value((int)&iss->eip, 4, FALSE);
	    } else {
		register struct i386_kernel_state *iks = STACK_IKS(kstack);
		frame = (struct i386_frame *)
		    db_get_value((int)&iks->k_ebp, 4, FALSE);
		callpc = (db_addr_t)db_get_value((int)&iks->k_eip, 4, FALSE);
	    }
	}
	else {
