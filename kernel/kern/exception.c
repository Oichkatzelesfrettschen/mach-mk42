	...
	int exception, code, subcode;
{
	 ...
	 *	and we have access to self->ith_saved.
	 ...
	 *	dest_port->ip_msgcount.  We must prepare
	 *	for the possibility of a hand-off.
	 */

	ip_unlock(dest_port);
	self->swap_func = exception_raise_continue;
	...
	exc->Head.msgh_kind = MACH_MSGH_KIND_NORMAL;
	...
	if ((receiver->ith_saved != IKM_NULL) ||
	    copyoutmsg((char *) &kmsg->ikm_header, (char *) receiver->ith_msg,
	...
	exception_raise_continue_slow(mr, reply_kmsg);
	/*NOTREACHED*/
    }
}
	...
	ipc_kmsg_t kmsg;
{
	...
	if ((kmsg->ikm_size == IKM_SAVED_KMSG_SIZE) &&
	    ((self = current_thread())->ith_saved == IKM_NULL))
		self->ith_saved = kmsg;
	else
		ikm_free(kmsg);
...
exception_raise_continue()
{
	...
	exception_raise_continue_slow(mr, kmsg);
	/*NOTREACHED*/
}
