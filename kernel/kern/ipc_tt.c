	...
	task_t parent;
{
	...
	if (kr != KERN_SUCCESS)
		panic("ipc_task_init: space");

	kport = ipc_port_alloc_kernel();
	if (kport == IP_NULL)
		panic("ipc_task_init: kport");
	...
	thread_t thread;
{
	...
	if (kr != KERN_SUCCESS)
		panic("ipc_thread_init: reply port");
	...
	thread_t thread;
{
	...
	/* free messages associated with this thread */

	assert(ipc_kmsg_queue_empty(&thread->ith_messages));
	if (thread->ith_saved != IKM_NULL)
		ikm_free(thread->ith_saved);
 ...
 *	Purpose:
 *		Convert from a port (really ipc_port_t) to a task.
...
task_t
convert_port_to_task(name)
	mach_port_t name;
{
	ipc_port_t port = (ipc_port_t) name;
 ...
 *	Purpose:
 *		Convert from a port (really ipc_port_t) to a space.
 ...
 *	Purpose:
 *		Convert from a port (really ipc_port_t) to a map.
 ...
 *	Purpose:
 *		Convert from a port (really ipc_port_t) to a thread.
 ...
 *	Purpose:
 *		Convert from a task to a port (really ipc_port_t).
 ...
 *	Purpose:
 *		Convert from a thread to a port (really ipc_port_t).
