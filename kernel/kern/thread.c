 ...
 *		stack_statistics
 *
 *	The thread->kernel_stack field is reserved for machine-dependent code.
 */
#else	MACHINE_STACK
/*
	...
	register thread_t thread;
{
	...
	if (stack == 0) {
		stack = kmem_alloc_wired(kernel_map, KERNEL_STACK_SIZE);
		if (stack == 0)
			panic("stack_alloc");
...
void thread_init()
{
	...
	thread_template.swap_state = TH_SW_OUT;
	...
	boolean_t		must_halt;
{
	 ...
	 *	   We use thread_swapout to discard the thread's stack.
	 *	   This assumes the thread was interrupted in user code.
	...
	switch(thread->state) {
		...
		if (thread->interruptible) {
			...
			if (thread->swap_state == TH_SW_IN)
			    thread_swapout(thread, thread_exception_return);
#endif	KEEP_STACKS
			break;
		    }
#if	NCPUS > 1
	...
	while (!queue_end(&all_psets, (queue_entry_t) pset)) {
		...
		while (!queue_end(&pset->threads, (queue_entry_t) thread)) {
			...
			if ((thread->state & (TH_RUN|TH_SWAPPED))
				...
				/*
				 *	Because the thread is swapped,
				 *	it can't be executing.  Hence
				 *	it is safe for us to grab the
				 *	cached kmsg away from it.
				 */

				kmsg = thread->ith_saved;
