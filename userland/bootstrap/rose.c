...
int rose_recog(struct file *, objfmt_t, char **);
int rose_load(struct file *, objfmt_t, char *);
...
int
rose_recog(struct file *fp, objfmt_t ofmt, char **hdr)
{
...
int
rose_load(struct file *fp, objfmt_t ofmt, char *hdr)
{
     ...
     */
    result = read_file(fp, ntohl(headerp->rmoh_load_map_cmd_off), (vm_offset_t)&ldc_header, sizeof(ldc_header));
    if (result)
	return result;

    load_cmd_map = (load_cmd_map_command_t *)malloc(ldc_header.ldci_cmd_size);
    result = read_file(fp, ntohl(headerp->rmoh_load_map_cmd_off), (vm_offset_t)load_cmd_map, ldc_header.ldci_cmd_size);
    if(result)
	return result;
	...
	case LDC_ENTRY:	        /* load command for the program main entry point (no section) */
	{
	    ...
	    result = read_file(fp, *ldc_off, (vm_offset_t)&entry_reg_cmd, sizeof(entry_command_t));
	    if(result)
		return result;
	...
	case LDC_REGION:	/* load command for a region section (part of the program)  */
	{
	    ...
	    result = read_file(fp, *ldc_off, (vm_offset_t)&load_reg_cmd, sizeof(region_command_t));
	    if(result)
		return result;
	     ...
	     objfmt_t ofmt)
{
    ...
    *(int*)nl = lp->str_size + sizeof(int);
     ...
     */
    result = read_file(fp, lp->str_offset, strings, lp->str_size); 
    if (result) 
	 ...
	 */
	result = host_load_symbol_table(host_port, task, symtab_name, symtab, table_size);
    }
