...
void free_file_buffers(struct file *);
int read_inode(ino_t, struct file *);
int block_map(struct file *, daddr_t, daddr_t *);
int buf_read_file(struct file *, vm_offset_t, vm_offset_t *, vm_size_t *);
int search_directory(char *, struct file *, ino_t *);
int read_fs(mach_port_t, struct fs **);
int mount_fs(struct file *);
void unmount_fs(struct file *);

/*
 * Free file buffers, but don't close file.
 */
void
free_file_buffers(register struct file *fp)
{
	register int level;
 ...
 */
int
read_inode(ino_t inumber, register struct file *fp)
{
	vm_offset_t		buf;
	vm_size_t		buf_size;
 ...
 */
int
block_map(struct file *fp, daddr_t file_block, daddr_t *disk_block_p)
{
	int		level;
	int		idx;
 ...
 */
int
buf_read_file(register struct file	*fp,
	      vm_offset_t		offset,
 ...
 */
int
search_directory(char *name, register struct file *fp, ino_t *inumber_p)
{
	vm_offset_t	buf;
	vm_size_t	buf_size;
	vm_offset_t	offset;
...
int
read_fs(mach_port_t dev, struct fs **fsp)
{
	register struct fs *fs;
	vm_offset_t	buf;
	vm_size_t	buf_size;
...
int
mount_fs(register struct file *fp)
{
	register struct fs *fs;
	int error;
...
void
unmount_fs(register struct file *fp)
{
	if (fp->f_fs != 0) {
	    (void) vm_deallocate(mach_task_self(),
				 (vm_offset_t) fp->f_fs,
 ...
 */
int
open_file(mach_port_t master_device_port, const char *path, struct file *fp)
{
	...
	while (*cp) {
	    ...
	    *cp = c;
		...
		if ((fp->f_di.di_flags & IC_FASTLINK) != 0) {
		    bcopy(fp->f_di.di_symlink, namebuf,
			(unsigned) link_len);
		}
		else
#endif	/* IC_FASTLINK */
